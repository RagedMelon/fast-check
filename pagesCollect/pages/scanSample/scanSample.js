...
Page({
    ...
    getTasks() {
        ...
        console.log(postData, '传入的参数')
        apiTask.getTaskList(postData).then(res => {
            wx.hideLoading()
            let Result = res.data.rows
            console.log(Result)
            console.log(res)
    ...
    selected(e) {
        ...
        console.log(this.data.missionId, '任务id')
    },
    // 档口名称选择
    showBoothList() {
        this.setData({
    ...
    selectedBusinessMan(e) {
        ...
        console.log(this.data.businessmanId, '选中的经营者id')
    ...
    chooseProductName(event) {
        ...
        saveData[0].businessProductId = productId
        this.setData({
            sampleName: event.currentTarget.dataset.name,
            showSearch: false,
            saveData: saveData
        })
        console.log(saveData, '样品id录入')
    ...
    bindDateChange: function(e) {
        console.log('进货时间为', e.detail.value)
        let saveData = this.data.saveData
    ...
    bindDateChange2: function(e) {
        console.log('采样时间为', e.detail.value)
        let saveData = this.data.saveData
        saveData[0].sampleDate = e.detail.value
        this.setData({
                ...
                if (res.code == 0) {
                    wx.showModal({
                        ...
                        success() {
                            let success = 1
                            wx.navigateTo({
                                url: '/pagesCollect/pages/samplingManager/samplingManager?success=' + success,
                            })
    ...
    onLoad: function(options) {
        ...
        if (options.dataObj) {
            this.setData({
                marketId: parseInt(options.marketId)
            })
            ...
            this.setData({
                marketId: parseInt(sampleData.boothInfoDto.marketId)
            })
            this.getMarketTasks(parseInt(sampleData.boothInfoDto.marketId))
            if (sampleData.material) {
                console.log(sampleData.material)
            ...
            this.setData({
                ...
                saveData: [{
                    ...
                    "place": "", //产地
                    "supplier": "", //供应商
                    "sampleCode": options.samplecode, //采样码
                    "businessProductId": "", //商品id
                    "material": "", //照片id-多张用,号隔开
                    "sampleDate": "" //采样时间
                }]
            })
            console.log(this.data.saveData, '样品码获取成功')
        }
